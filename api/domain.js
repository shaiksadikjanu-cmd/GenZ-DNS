// api/domain.js — public domain viewer
// Routes:
//   /u/:username/:domain → JanuNet default (our Firestore)
//   /s/:ref/:domain      → User's Supabase (lookup anon key by project ref)
//   /f/:projectId/:domain → User's Firebase (direct Firestore query)

import { getStorage } from '../public/lib/storage/index.js';

export default async function handler(req, res) {
  const { domain, backend, username, ref, projectId } = req.query;
  ...
  if (backend === 'supabase' && ref) {
    storage = await getSupabaseStorage(ref);
    if (!storage) {
      return res.status(404).send(errorPage(`No Supabase workspace found for ref: ${ref}`));
    }
  ...
  let domainDoc;
  try {
    domainDoc = await storage.getDomain(domain);
  } catch(e) {
    console.error('storage error:', e);
    return res.status(500).send(errorPage('Could not reach storage backend.'));
  }

  if (!domainDoc) {
    return res.status(404).send(errorPage(`Domain not found: ${domain}`));
  }

  // Increment visits fire-and-forget
  storage.incrementVisits(domain).catch(() => {});

  res.setHeader('Content-Type', 'text/html');
  res.status(200).send(viewerPage({
    domainName: domainDoc.name,
    targetUrl:  domainDoc.targetUrl,
    visits:     domainDoc.visits + 1,
    ownerName:  domainDoc.ownerName || username || 'unknown'
  }));
}

// Look up user's Supabase anon key by project ref
// We store this in janu_users when user saves workspace
async function getSupabaseStorage(ref) {
  const FIRESTORE = `https://firestore.googleapis.com/v1/projects/janunet-cloud/databases/(default)/documents`;

  // Query janu_users where publicSupabaseRef == ref
  const q = {
    structuredQuery: {
      from:  [{ collectionId: 'janu_users' }],
      where: {
        fieldFilter: {
          field: { fieldPath: 'publicSupabaseRef' },
          op:    'EQUAL',
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .nav-btn{width:28px;height:28px;border:none;background:transparent;color:var(--text2);
    cursor:pointer;border-radius:6px;display:inline-flex;align-items:center;
    justify-content:center;transition:background 120ms,color 120ms;}
  .nav-btn:hover{background:var(--surface2);color:var(--text);}
  .nav-btn svg{width:16px;height:16px;stroke-width:2;}
  .urlbar{height:30px;background:var(--surface2);border:1px solid var(--border);
    border-radius:999px;display:flex;align-items:center;padding:0 14px 0 12px;
  ...
  .url-text{flex:1;min-width:0;display:flex;align-items:baseline;gap:8px;
  ...
  .url-target{color:var(--text2);font-size:11px;white-space:nowrap;
    overflow:hidden;text-overflow:ellipsis;min-width:0;}
  .right-group{display:flex;align-items:center;gap:8px;padding-right:4px;flex-shrink:0;}
  .visits{font-family:var(--font-mono);font-size:11px;color:var(--text3);
    display:flex;align-items:center;gap:4px;}
  .visits b{color:var(--text2);}
  .open-btn{height:28px;padding:0 12px;background:var(--accent-dim);
    border:1px solid transparent;color:var(--accent);border-radius:6px;cursor:pointer;
    font-size:12px;font-weight:600;display:inline-flex;align-items:center;gap:6px;
    transition:background 120ms,border-color 120ms;text-decoration:none;white-space:nowrap;}
  .open-btn:hover{background:rgba(0,255,204,0.14);border-color:rgba(0,255,204,0.3);}
  .brand{font-family:var(--font-mono);font-size:11px;color:var(--text3);
    letter-spacing:1px;text-transform:uppercase;}
  .brand b{color:var(--accent);}
  .frame-wrap{height:calc(100% - 44px);position:relative;background:#fff;}
  iframe{width:100%;height:100%;border:none;background:#fff;}
  .loading-bar{position:absolute;top:0;left:0;height:2px;background:var(--accent);
    width:0%;box-shadow:0 0 6px var(--accent);z-index:5;transition:width 300ms;}
  ...
  .blocked{position:absolute;inset:0;background:var(--bg);display:none;
  ...
  .blocked a{display:inline-block;padding:12px 24px;background:var(--accent);
    color:#000;border-radius:8px;font-weight:700;font-size:13px;text-decoration:none;}
...
<div class="topbar">
  ...
  <div class="right-group">
    ...
    <a class="open-btn" href="${targetUrl}" target="_blank" rel="noopener">
...
<div class="frame-wrap">
  ...
  <div class="blocked" id="blocked">
    ...
    <p>${domainName} routes to <b>${displayTarget}</b>, but this site blocks embedding.</p>
